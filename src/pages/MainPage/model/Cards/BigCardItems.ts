import CkeckIcon from '@/shared/assets/circle-check.svg';
import CalendarIcon from '@/shared/assets/calendar-card.svg';
import LayoutIcon from '@/shared/assets/layout.svg';

export const BIG_CARD_ITEMS = [
    {
        id: '1',
        icon: CkeckIcon,
        title: 'Умное управление задачами',
        text: 'Создавайте задачи с подробным описанием, цветными метками и отслеживанием выполнения. Ваши дела ещё никогда так не выглядели.',
    },
    {
        id: '2',
        icon: CalendarIcon,
        title: 'Блоки времени',
        text: 'Назначайте время начала и окончания, превращая задачи в полноценные блоки концентрации. В виде «День» задачи отображаются на визуальной временноой сетке — вернитe контроль над расписанием.',
    },
    {
        id: '3',
        icon: LayoutIcon,
        title: 'Гибкие виды',
        text: 'Переключайтесь между «День» и «Список». Смотрите общую картину или фокусируйтесь на сегодняшних приоритетах.',
        // TODO: Вернуть когда буду добавлять Неделю и Месяц
        // text: 'Переключайтесь между «День», «Неделя», «Месяц» и «Список». Смотрите общую картину или фокусируйтесь на сегодняшних приоритетах.',
    },
];
